...
data "azurecaf_name" "avd_workspace" {
  ...
  prefixes      = compact([local.name_prefix, "vdws"])
...
data "azurecaf_name" "avd_host_pool" {
  ...
  prefixes      = compact([local.name_prefix, "vdpool"])
  suffixes      = compact([var.client_name, var.location_short, var.environment, local.name_suffix])
  use_slug      = false
  clean_input   = true
  separator     = "-"
...
data "azurecaf_name" "avd_app_group" {
  ...
  prefixes      = compact([local.name_prefix, "vdag"])
  suffixes      = compact([var.client_name, var.location_short, var.environment, local.name_suffix])
  use_slug      = false
  clean_input   = true
  separator     = "-"
...
data "azurecaf_name" "avd_scaling_plan" {
  ...
  prefixes      = compact([local.name_prefix, "vdscaling"])
  suffixes      = compact([var.client_name, var.location_short, var.environment, local.name_suffix])
  use_slug      = false
  clean_input   = true
  separator     = "-"
